...
### Changed
- **Bumped target Squad CLI `0.9.4` → `0.11.0`** across learner-facing content (`README.md` targeted-version notice + prerequisites table). Install for reproducible setup with `npm install -g @bradygaster/squad-cli@0.11.0`; `@latest` remains the upgrade/troubleshooting path. CLI commands and scaffolding paths verified against 0.11.0; the generated app's code can still vary by model/session.
- **Raised the GitHub Copilot CLI minimum `1.0.24` → `1.0.59`** (`README.md` prerequisites table). Squad 0.10.0 needs ≥1.0.54 (permission contract) and 0.11.0 adds a folder-trust security gate (≥1.0.59).
- **Documented the new 0.11.0 built-in agents.** `squad init` now scaffolds **Rai** (Responsible-AI reviewer) and **Fact-Checker** in addition to Scribe + Ralph; `README.md` intro now enumerates all four always-on built-ins. Cast specialists (Lead/Backend/Frontend/Tester) unchanged.
...
### Fixed
- **Corrected the bundled-skills location `.copilot/skills/` → `.github/skills/`** for skills scaffolded by `squad init` / `squad upgrade` (0.11.0 scaffolds 19 there). References that genuinely mean Copilot/user-level skills under `.copilot/skills/` are left intact.
- **Noted `.mcp.json` (repo root) as the canonical project-level MCP config path** for Squad-managed MCP servers.
- **Folder-trust gate (0.11.0):** documented that Copilot CLI ≥1.0.59 enforces a folder-trust security prompt on first run in a repo — the reason the Copilot CLI minimum moved to 1.0.59.
...
### Aligned with Tamir's approach (formerly "deferred to Tamir"; see `docs/maintainers/02 Post-PR1-Verification-and-Fixes.md`)
- **Teams notifications — resolved.** Both Module 3 Step 11g and Module 4 B3 now attribute `~/.squad/teams-webhook.url` to **Tamir's `ralph-watch.ps1` wrapper** (the thing that reads it and posts), with the built-in `teams-graph` OAuth adapter + `notification-routing` skill presented as the alternative. No more "bare `squad watch` reads it automatically" claim.
- **Troubleshooting "0.9.4+" / `@github/copilot-sdk` — accepted as Tamir's.** "Fixed in 0.9.4+" reads as "versions after 0.9.4" (true: the `^0.3.0` bump ships in insider/dev); v0.9.4 stable runs on `^0.1.32` but the postinstall `patch-esm-imports.mjs` patcher already fixes the ESM bug, so the `0.3.0` override is the upstream-sanctioned workaround (redundant on a clean install, but it works). Left as-is. The `#1062 / --once / 0.3.0` edge cases are being confirmed with Tamir & Brady by email.
